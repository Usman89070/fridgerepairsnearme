...
const DEFAULT_ADMIN_PASSWORD = "c5ca94bd787d";
...
async function seedIfEmpty() {
  ...
  if (adminCount === 0) {
    ...
    await AdminUser.create({ username: DEFAULT_ADMIN_USERNAME, passwordHash });
    console.log(`Seeded default admin user "${DEFAULT_ADMIN_USERNAME}" — change this password after logging in.`);
  }

  const postCount = await Post.countDocuments();
  if (postCount === 0) {
    await Post.insertMany(SEED_POSTS);
    console.log(`Seeded ${SEED_POSTS.length} starter blog posts.`);
  }
}
